...
public abstract class AbstractDocumentProperty<THIS extends AbstractDocumentProperty<? super THIS>> 
    ...
    protected AbstractDocumentProperty(Map<String, Object> data) {
        ...
        config     = requireNonNull(data);
        properties = observableMap(new ConcurrentHashMap<>());
    ...
    /**
     * An overridable method used to select a constructor for children under a
     * specified key.
     * 
     * @param key  the key
    ...
    private <T> Property<T> addListeners(String key, Property<T> property) {
        property.addListener((ob, o, n) -> {
            config.put(key, property.getValue());
        });
