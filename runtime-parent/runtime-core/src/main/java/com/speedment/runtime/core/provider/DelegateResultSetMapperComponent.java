...
public final class DelegateResultSetMapperComponent implements ResultSetMapperComponent {
    ...
    private final ResultSetMapperComponent inner;

    public DelegateResultSetMapperComponent() {
        this.inner = new ResultSetMapperComponentImpl();
    }

    @Override
    public <T> ResultSetMapping<T> apply(DbmsType dbmsType, Class<T> javaClass) {
        return inner.apply(dbmsType, javaClass);
    }

    @Override
    public <T> ResultSetMapping<T> apply(Class<T> javaClass) {
        return inner.apply(javaClass);
    }
}
